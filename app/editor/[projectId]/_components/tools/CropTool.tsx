...
const ASPECT_RATIOS = [
  { labelKey: 'aspectRatios.freeform', value: null, icon: Maximize },
  { labelKey: 'aspectRatios.square', value: 1, icon: Square, ratio: '1:1' },
  {
    labelKey: 'aspectRatios.widescreen',
    value: 16 / 9,
    icon: RectangleHorizontal,
    ratio: '16:9',
  },
  { labelKey: 'aspectRatios.portrait', value: 4 / 5, icon: RectangleVertical, ratio: '4:5' },
  { labelKey: 'aspectRatios.story', value: 9 / 16, icon: Smartphone, ratio: '9:16' },
]

function CropTool() {
  const t = useTranslations('editor.tools.crop')
  ...
  const applyCrop = async () => {
    ...
    try {
      ...
      alert(t('errors.applyCropFailed'))
      exitCropMode()
    }
  }
  ...
  if (!activeImage && !isCropMode) {
    return (
      <div className="p-4">
        <p className="text-white/70 text-sm">{t('selectImageToCrop')}</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {isCropMode && (
        <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-3">
          <p className="text-cyan-400 text-sm font-medium">✂️ {t('cropModeActive')}</p>
          <p className="text-cyan-300/80 text-xs mt-1">{t('cropModeActiveDescription')}</p>
        </div>
      )}
      ...
      {isCropMode && (
        <div>
          <h3 className="text-sm font-medium text-white mb-3">{t('cropAspectRatios')}</h3>
            ...
            {ASPECT_RATIOS.map(ratio => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs text-white">{t(ratio.labelKey)}</div>

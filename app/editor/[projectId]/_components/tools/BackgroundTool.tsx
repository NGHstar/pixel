...
import { Trash2, Palette, Image as ImageIcon, Search, Download, Loader2, Check } from 'lucide-react'
...
export default function BackgroundTool({ project }: { project: Doc<'projects'> }) {
  const t = useTranslations('editor.tools.background')
  ...
  const handleBackgroundRemoval = async () => {
    ...
    setProcessingMessage(t('processing.removingBackground'))
    ...
    try {
      ...
      alert(t('errors.removeBackground'))
    } finally {
  ...
  const searchUnsplashImages = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('errors.searchImages'))

      const data = await response.json()
      setUnsplashImages(data.results || [])
    } catch (error) {
      console.error('Error searching Unsplash:', error)
      alert(t('errors.searchImages'))
    } finally {
  ...
  const handleImageBackground = async (imageUrl, imageId) => {
    ...
    try {
      ...
      alert(t('errors.setBackgroundImage'))
      setSelectedImageId(null)
    }
  }
  ...
  if (!canvasEditor) {
    return (
      <div className="p-4">
        <p className="text-white/70 text-sm">{t('canvasNotReady')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 relative h-full">
      ...
      <div className="space-y-4 pb-4 border-b border-white/10">
        <div>
          <h3 className="text-sm font-medium text-white mb-2">{t('title')}</h3>
        ...
        {!getMainImage() && <p className="text-xs text-amber-400">{t('noImageNotice')}</p>}
      </div>

      {/* Shadcn UI Tabs */}
      <Tabs defaultValue="color" className="w-full">
        <TabsList className="grid w-full grid-cols-2 bg-slate-700/50 px-1 gap-1">
          <TabsTrigger
            value="color"
        ...
        <TabsContent value="color" className="space-y-4 mt-3">
          <div className="rtl:text-right">
            <h3 className="text-sm font-medium text-white mb-2">{t('tabs.color.title')}</h3>
            <p className="text-xs text-white/70 mb-4">{t('tabs.color.description')}</p>
          </div>
        ...
        <TabsContent value="image" className="space-y-4 mt-6">
          <div>
            <h3 className="text-sm font-medium text-white mb-2">{t('tabs.image.title')}</h3>
            <p className="text-xs text-white/70 mb-4">{t('tabs.image.description')}</p>
          </div>
          ...
          {unsplashImages?.length > 0 && (
            <div className="space-y-3">
              <h4 className="text-sm font-medium text-white">
                {t('searchResults', { count: unsplashImages?.length || 0 })}
              </h4>
                ...
                {unsplashImages.map(image => (
                  ...
                  >
                    <img
                      ...
                      alt={image.alt_description || t('imageAlt')}
          ...
          {!isSearching && unsplashImages?.length === 0 && searchQuery && (
            <div className="text-center py-8">
              ...
              <p className="text-white/70 text-sm">{t('noImagesFound', { query: searchQuery })}</p>
              <p className="text-white/50 text-xs">{t('tryDifferentSearch')}</p>
            </div>
          )}
          ...
          {!searchQuery && unsplashImages?.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-white/70 text-sm">{t('searchPrompt')}</p>
              <p className="text-white/50 text-xs">{t('poweredByUnsplash')}</p>
            </div>
          )}
          ...
          {!UNSPLASH_ACCESS_KEY && (
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
              <p className="text-amber-400 text-xs">{t('errors.unsplashApiKeyMissing')}</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
